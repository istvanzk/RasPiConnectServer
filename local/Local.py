...
# FAB switch button state
# See FAB_func(.) and LED_func(.)
FAB_SwitchONOFF = False
...
# OSC msg sender (client)
from pythonosc import osc_message_builder
from pythonosc import udp_client
OSCclient = udp_client.UDPClient("192.168.87.112", 1515) # Remote Server IP/Port
def sendOSC(osc_address, osc_val):
	"""
	Send an OSC message.
	"""
	if TXOSC:
		try:
			...
			if LocalDEBUG:
				print("sendOSC:: Sent message {} to address {}".format(osc_val, osc_address))
				
		except:
			raise
		
# OSC msg receiver (server)
from pythonosc import dispatcher
from pythonosc import osc_server    
def receiveOSC():
	"""
	Start the OSC dispatcher server.
	This function has to be run in thread in order not to block the execution.
	"""	
	try:
		OSCdispatcher = dispatcher.Dispatcher()
		OSCdispatcher.map("/debug", print)
		#OSCdispatcher.map("/volume", print_volume_handler, "Volume")
		#OSCdispatcher.map("/logvolume", print_compute_handler, "Log volume", math.log)

		OSCserver = osc_server.ThreadingOSCUDPServer(("192.168.87.111", 1516), OSCdispatcher) # Local Server IP/Port
	
		if LocalDEBUG:
			print("receiveOSC:: Serving on {}".format(OSCserver.server_address))
...
def AssignObjectServerHandlers():
	...
	ObjectServerHandlers.map('mWP-1', RWP_func, 'ThingSpeak figure', 1)
	ObjectServerHandlers.map('mWP-2', RWP_func, 'Dropbox cam 1 image', 2)
	ObjectServerHandlers.map('mWP-3', RWP_func, 'Dropbox cam 2 image', 3)
	ObjectServerHandlers.map('mBR-1', BAR_func, 'Used memory fraction', 1)
	ObjectServerHandlers.map('mFB-1', FAB_func, 'Feedback button with mL-1', 1)
	ObjectServerHandlers.map('mFB-2', FAB_func, 'Reboot button', 2)
	ObjectServerHandlers.map('mL-1', LED_func, 'Feedback LED for mFB-1', 1)
	ObjectServerHandlers.map('mSB-1', SAB_func, 'Simple button', 1)
	ObjectServerHandlers.map('mST-1', STS_func, 'Send OSC message', 1)
...
def RWP_func(list_par, bValidate):
	"""HTML/iframe page with a picture"""
	...
	if list_par[0] == 1:
		...
		responseData += "<head><title>ThingSpeak plot</title><style>body,html,iframe{margin:0;padding:0;}</style></head>"

		#responseData += "<body><iframe width=\"350\" height=\"300\" src=\"http://api.thingspeak.com/channels/9981/charts/2?width=320&height=300&timescale=10&days=2&dynamic=true\" ></iframe>"
		responseData += "<body><iframe width=\"250px\" height=\"260px\" src=\"http://api.thingspeak.com/apps/matlab_visualizations/24433\" ></iframe></body>"

	# Dropbox cam 1 image
	elif list_par[0] == 2:

		responseData += "<head><title>Cam1 image</title><style>body,html,iframe{margin:0;padding:0;}</style></head>"

		responseData += "<body><img width=\"350px\" src=\"http://www.dropbox.com/s/02hg3y81wsglvqe/cam.jpg?raw=1\"></body>"

	# Dropbox cam 2 image
	elif list_par[0] == 3:

		responseData += "<head><title>Cam2 image</title><style>body,html,iframe{margin:0;padding:0;}</style></head>"

		responseData += "<body><img width=\"350px\" src=\"https://www.dropbox.com/s/rlt337ywaizmexi/cam2.jpg?raw=1\"></body>"

	responseData += "</html>"
	return responseData


def BAR_func(list_par, bValidate):
	"""Bar meter"""
	...
	if list_par[0] == 1:
		...
		freeString = subprocess.check_output(["free", "-mt"]).strip().decode('utf-8')
		freeSplit = freeString.split("\nTotal: ", 1)
		freeSplit = freeSplit[1].split("       ", 3)
		freeData = 100.0*float(freeSplit[2])/float(freeSplit[1])
		responseData = "%f" % freeData

	return responseData


def FAB_func(list_par, bValidate):
	...
	# Button for raspi reboot cmd
	elif list_par[0] == 2:

		lowername = objectName.lower()

		if (lowername.count("run") > 0):

			lowername = lowername.replace("run", "reboot")
			responseData = lowername.title()
...
def LED_func(list_par, bValidate):
	"""LED"""
...
def SAB_func(list_par, bValidate):
	...
	# Send a pre-defined OSC message
	# The OSC address and value are provided in the objectName string
	if list_par[0] == 1:
		
		arg_str = list_par[1].split("osc: ",1)
		cmd_str = arg_str[1].split("/",1)
		
		addr = "/{}".format( cmd_str[0] )
		
		sendOSC( addr, cmd_str[1] )
		
		responseData = "OK"
		
	return responseData

def STS_func(list_par, bValidate):
	"""Send to server"""
	...
	# The OSC address and value are provided in the objectAction string	
	if list_par[0] == 1:
	
		cmd_str = list_par[2].split("/",1)
...
if RXOSC:
	import threading
	server_thread = threading.Thread(target=receiveOSC, args=())
	server_thread.start()
